/**
 * นำเข้าทะเบียนครุภัณฑ์ประเภทวิทยุสื่อสาร (จากสเปรดชีต)
 * รัน: npx tsx scripts/seed-radio-assets.ts
 * หรือ: npm run seed:radio
 *
 * ถ้าเลขครุภัณฑ์มีอยู่แล้ว จะอัปเดตชื่อรายการ / ที่ตั้ง / ประเภท / หมายเหตุ ให้ตรงชุดนี้
 */
...
/** ข้อมูลตามตาราง (Asset No. → serialNumber) */
const RADIO_ROWS: Row[] = [
  ...
  {
    serial: "1320000635",
    itemName: DESC_MOBILE_COMPACT,
    costCenter: "103801",
    location: "นอกอาคาร สนง.",
    brand: "hysera",
  },
  {
    serial: "1320000636",
  ...
  ...(
    ...
    (serial) =>
      ({
        serial,
        itemName: DESC_HANDHELD,
        costCenter: "103801",
        location: "อาคาร 1 ชั้น 1 โซน 1",
        brand: "DRC",
      }) satisfies Row,
  ),
...
async function main() {
  ...
  for (const row of RADIO_ROWS) {
    ...
    if (existing) {
      ...
      updated++;
      console.log(`อัปเดต ${serialNumber}`);
    } else {
      ...
      created++;
      console.log(`เพิ่ม ${serialNumber}`);
    }
  }

  console.log(`\nเสร็จ: เพิ่ม ${created} รายการ, อัปเดต ${updated} รายการ (รวม ${RADIO_ROWS.length} แถว)`);
}
