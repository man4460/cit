/**
 * นำเข้าบุคลากร / ค่าตอบแทน / น้ำมันรายคัน เข้า TRIP-2569-04 (27–28 เม.ย.69 ศรย.)
 *
 * ไฟล์:
 *  - รายละเอียดค่าใช้จ่าย_ศรย_27_28_เม_ย_69.xlsx  (ชีต ทะเบียนน้ำมัน)
 *  - ค่าตอบแทน ฝรภ (2).xlsx
 *  - ค่าตอบแทน ตำรวจ (2).xlsx
 *
 * บุคลากร: จับคู่รหัส/ชื่อที่มีอยู่แล้ว — ไม่สร้างซ้ำ
 * น้ำมัน: แยกตามทะเบียนรถ — หลายคันในแถวหารลิตรเท่า ๆ กัน
 *
 *   npm run seed:missions:apr69-rayong
 */
...
const MISSION_CODE = "TRIP-2569-04";
const TAG = "[เมย69-ศรย]";

const EXPENSE_XLSX =
  process.env.APR69_RAYONG_EXPENSE_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\รายละเอียดค่าใช้จ่าย_ศรย_27_28_เม_ย_69.xlsx`;
const FARAB_XLSX =
  process.env.APR69_RAYONG_FARAB_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\ค่าตอบแทน ฝรภ (2).xlsx`;
const POLICE_XLSX =
  process.env.APR69_RAYONG_POLICE_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\ค่าตอบแทน ตำรวจ (2).xlsx`;
...
for r in range(1, ws.max_row + 1):
    ...
    if not ds or "/" not in ds:
        continue
    # เฉพาะ 27–28 เม.ย.69 (ไม่ดึงทริป ศพล-ศชม ที่ 21–23)
    if not (ds.startswith("27/4") or ds.startswith("28/4") or ds.startswith("27/04") or ds.startswith("28/04")):
        continue
    fuel.append({
        "dateText": ds,
...
`.trim();
  ...
  const scriptPath = path.join(os.tmpdir(), `export-apr69-rayong-${Date.now()}.py`);
...
async function main() {
  ...
  for (const row of data.farab) {
    ...
    const idNumber = row.empCode || `SEED-FARAB-APR69-RAYONG-${row.seq}`;
  ...
  for (const row of data.police.filter((p) => p.isPerson)) {
    const fullName = `${row.firstName} ${row.lastName}`.replace(/\s+/g, " ").trim();
    const idNumber =
      row.idNumber.length >= 12 ? row.idNumber : `SEED-POLICE-APR69-RAYONG-${row.seq}-${row.idNumber}`;
    const { id: personnelId, reused } = await ensurePersonnel({
      fullName,
      idNumber,
      rank: row.rank,
    ...
    if (fuelExpenseType) {
      ...
      await tx.missionExpense.create({
        data: {
          ...
          amount: new Prisma.Decimal(data.fuelTotalBaht || 11550),
          description: `${TAG} ค่าน้ำมันตามทะเบียน (${vehicleLinks.length} คัน)`,
          incurredAt: mission.plannedStart ?? new Date(Date.UTC(2026, 3, 27, 12)),
        },
      });
    }
