/**
 * นำเข้าบุคลากร / ค่าตอบแทน / น้ำมันรายคัน เข้า TRIP-2569-01 (6–8 ม.ค.69 ศนร.+ศขก.)
 *
 * ไฟล์:
 *  - รายละเอียดค่าใช้จ่าย_ศนร+ศขก_6_8_ม_ค_69.xlsx  (ทะเบียนน้ำมัน)
 *  - ค่าตอบแทน ฝรภ..xlsx
...
const MISSION_CODE = "TRIP-2569-01";
const TAG = "[มค69-ศนรศขก]";

const EXPENSE_XLSX =
  process.env.JAN69_EXPENSE_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\รายละเอียดค่าใช้จ่าย_ศนร+ศขก_6_8_ม_ค_69.xlsx`;
const FARAB_XLSX =
  process.env.JAN69_FARAB_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\ค่าตอบแทน ฝรภ..xlsx`;
const POLICE_XLSX =
  process.env.JAN69_POLICE_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\ค่าตอบแทน ตำรวจ..xlsx`;
...
for r in range(1, ws.max_row + 1):
    ...
    b = ws.cell(r, 2).value
    c = ws.cell(r, 3).value
    # section header (no seq number in col1 as int, but text unit name spanning)
    ...
    # คนจริง: มีชื่อและนามสกุล และเลขบัตรยาวพอ
    is_person = bool(first and last and len(id_digits) >= 12)
    # แถวหน่วยงาน/vendor (ชื่อว่างหรือเป็นชื่อสถานี)
...
ws = wb["ทะเบียนควบคุมน้ำมัน"] if "ทะเบียนควบคุมน้ำมัน" in wb.sheetnames else wb["ทะเบียนน้ำมัน"]
fuel = []
for r in range(1, ws.max_row + 1):
    ...
    if date is None or plates_cell is None or liters is None:
        continue
    if not str(date).strip() or "/" not in str(date):
        continue
    fuel.append({
        "dateText": str(date).strip(),
...
`.trim();
  ...
  const scriptPath = path.join(os.tmpdir(), `export-jan69-${Date.now()}.py`);
...
async function main() {
  ...
  for (const row of data.farab) {
    ...
    const idNumber = row.empCode || `SEED-FARAB-JAN69-${row.seq}`;
    const personnelId = await ensurePersonnel({
      fullName,
      idNumber,
      rank: row.rank,
    ...
    console.log(`  ฝรภ. ${row.seq}. ${row.rank} ${fullName} · ${row.total}`);
  }

  for (const row of data.police.filter((p) => p.isPerson)) {
    const fullName = `${row.firstName} ${row.lastName}`.replace(/\s+/g, " ").trim();
    const idNumber = row.idNumber.length >= 12 ? row.idNumber : `SEED-POLICE-JAN69-${row.seq}-${row.idNumber}`;
    const personnelId = await ensurePersonnel({
      fullName,
      idNumber,
      rank: row.rank,
    ...
    console.log(`  ตร. ${row.seq}. ${row.rank} ${fullName} · ${row.amount}`);
  }

  // น้ำมันรายคัน (หารลิตรเมื่อหลายคันในแถวเดียวกัน)
  ...
  // อัปเดตคำอธิบายค่าน้ำมัน + เก็บยอดตามทะเบียน
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (fuelType) {
      await tx.missionExpense.deleteMany({
        where: { missionId: mission.id, expenseTypeId: fuelType.id },
      });
      await tx.missionExpense.create({
        data: {
          missionId: mission.id,
          expenseTypeId: fuelType.id,
          amount: new Prisma.Decimal(data.fuelTotalBaht || 16230),
          description: `${TAG} ค่าน้ำมันตามทะเบียนควบคุม (${vehicleLinks.length} คัน)`,
          incurredAt: mission.plannedStart ?? new Date(Date.UTC(2026, 0, 6, 12)),
        },
      });
    }
