/**
 * นำเข้าบุคลากร / ค่าตอบแทน / น้ำมันรายคัน เข้า TRIP-2569-03 (21–23 เม.ย.69 ศพล.+ศชม.)
 *
 * ไฟล์:
 *  - รายละเอียดค่าใช้จ่าย_ศพล_ศชม_วันที่_21_23_เม_ย_69.xlsx  (ชีต ทะเบียนน้ำมัน)
 ...
 *
 *   npm run seed:missions:apr69-sophon
 */
...
const MISSION_CODE = "TRIP-2569-03";
const TAG = "[เมย69-ศพลศชม]";

const EXPENSE_XLSX =
  process.env.APR69_EXPENSE_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\รายละเอียดค่าใช้จ่าย_ศพล_ศชม_วันที่_21_23_เม_ย_69.xlsx`;
const FARAB_XLSX =
  process.env.APR69_FARAB_XLSX ??
  String.raw`C:\Users\LENOVO\Downloads\Telegram Desktop\ค่าตอบแทน ฝรภ. (2).xlsx`;
const POLICE_XLSX =
  process.env.APR69_POLICE_XLSX ??
...
for r in range(1, ws.max_row + 1):
    ...
    if not ds or "/" not in ds:
        continue
    # กรองเฉพาะ เม.ย.69 ถ้าชีตสะสม
    if "/4/" not in ds and "/04/" not in ds:
        continue
    fuel.append({
        "dateText": ds,
...
`.trim();
  ...
  const scriptPath = path.join(os.tmpdir(), `export-apr69-${Date.now()}.py`);
    ...
    if (fuelExpenseType) {
      ...
      await tx.missionExpense.create({
        data: {
          ...
          amount: new Prisma.Decimal(data.fuelTotalBaht || 41152),
          description: `${TAG} ค่าน้ำมันตามทะเบียน (${vehicleLinks.length} คัน)`,
          incurredAt: mission.plannedStart ?? new Date(Date.UTC(2026, 3, 21, 12)),
        },
      });
    }
